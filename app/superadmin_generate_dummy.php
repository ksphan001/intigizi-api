...
try {
    ...
    // 2. Pastikan Dapur Demo (ID = 2) Tersedia
    ...
    // 3. Salin data dari master_ingredients ke ingredients untuk organisasi ID = 2
    $conn->query("INSERT INTO ingredients (organization_id, name, unit_id, latest_price, master_ingredient_id) 
        SELECT 2, name, 2, estimated_price, id FROM master_ingredients");

    // 4. Salin data nutrisi ke nutrition_data
    $conn->query("INSERT INTO nutrition_data (organization_id, ingredient_id, calories, protein, carbohydrates, fat, fiber, bdd_percentage) 
        SELECT 2, i.id, m.calories, m.protein, m.carbohydrates, m.fat, m.fiber, m.bdd_percentage 
    ...
    $getIngredientId = function($name) use ($conn) {
        $res = $conn->query("SELECT id FROM ingredients WHERE name = '" . $conn->real_escape_string($name) . "' AND organization_id = 2");
    ...
    foreach ($menusToCreate as $menuData) {
        $stmt = $conn->prepare("INSERT INTO menus (organization_id, menu_name, created_by) VALUES (2, ?, 5)"); // 5 = ahli-gizi
        ...
        foreach ($menuData['ingredients'] as $ingId => $qty) {
            ...
            $stmtDetails = $conn->prepare("INSERT INTO menu_ingredients (organization_id, menu_id, ingredient_id, quantity_per_portion) VALUES (2, ?, ?, ?)");
    ...
    $pointIds = [];
    foreach ($points as $p) {
        $stmt = $conn->prepare("INSERT INTO distribution_points (organization_id, name, address, latitude, longitude) VALUES (2, ?, ?, -6.29 + (rand()-0.5)*0.05, 106.81 + (rand()-0.5)*0.05)");
    ...
    foreach ($volunteerNames as $vName) {
        ...
        $stmt = $conn->prepare("INSERT INTO volunteers (organization_id, full_name, job_type, phone_number, address) VALUES (2, ?, ?, ?, 'Jakarta Selatan')");
    ...
    for ($i = 0; $i < 120; $i++) {
        ...
        $conn->query("INSERT INTO beneficiary_bmi_history (organization_id, beneficiary_id, measurement_date, weight_kg, height_cm, bmi, recorded_by_user_id) 
            VALUES (2, {$beneficiaryId}, DATE_SUB(CURDATE(), INTERVAL 60 DAY), " . ($weight - rand(2, 4)) . ", " . ($height - 2) . ", {$bmi}, 5)");
        $conn->query("INSERT INTO beneficiary_bmi_history (organization_id, beneficiary_id, measurement_date, weight_kg, height_cm, bmi, recorded_by_user_id) 
            VALUES (2, {$beneficiaryId}, DATE_SUB(CURDATE(), INTERVAL 30 DAY), " . ($weight - rand(1, 2)) . ", " . ($height - 1) . ", {$bmi}, 5)");
    }
    ...
    foreach ($stockItems as $ingId => $qty) {
        if (!$ingId) continue;
        $conn->query("INSERT INTO stock (organization_id, ingredient_id, current_quantity) VALUES (2, {$ingId}, {$qty}) ON DUPLICATE KEY UPDATE current_quantity = {$qty}");
        $conn->query("INSERT INTO stock_transactions (organization_id, ingredient_id, type, quantity, notes, transaction_date) 
            VALUES (2, {$ingId}, 'Masuk', {$qty}, 'Penerimaan Stok Awal', DATE_SUB(NOW(), INTERVAL 15 DAY))");
    }
    ...
    foreach ($proposals as $prop) {
        ...
        $conn->query("INSERT INTO proposals (organization_id, proposal_code, start_date, end_date, target_recipients, status, created_by) 
            VALUES (2, '{$prop['code']}', '{$startDate}', '{$endDate}', 150, '{$prop['status']}', 7)");
        ...
        for ($day = 0; $day < 7; $day++) {
            ...
            $conn->query("INSERT INTO proposal_menus (organization_id, proposal_id, menu_id, serving_date) VALUES (2, {$proposalId}, {$menuId}, '{$servingDate}')");
        }
            ...
            for ($day = 0; $day < 7; $day++) {
                ...
                foreach ($pointIds as $ptId) {
                    ...
                    $conn->query("INSERT INTO distribution_reports (organization_id, distribution_point_id, distribution_date, menu_id, quantity_sent, quantity_received, status, notes, reported_by) 
                        VALUES (2, {$ptId}, '{$distDate}', {$menuId}, 50, 50, 'Diterima', 'Distribusi selesai tanpa hambatan', 6)");
                }
            }
        }
    }
    ...
    foreach ($expenses as $exp) {
        ...
        $conn->query("INSERT INTO operational_expenses (organization_id, category_id, description, amount, expense_date, created_by, source_account_id) 
            VALUES (2, {$exp['cat']}, '{$exp['desc']}', {$exp['amount']}, '{$expDate}', 4, 1)"); // 1 = Kas Tunai
    }
    ...
    $conn->query("INSERT INTO financial_transactions (organization_id, transaction_date, description, debit_account_id, credit_account_id, amount, created_by) 
        VALUES (2, DATE_SUB(CURDATE(), INTERVAL 25 DAY), 'Penerimaan Modal Hibah Awal Kemenkes', 2, 7, 150000000.00, 4)"); // Debit Bank (2), Kredit Modal (7)

    // Pencairan ke Kas Tunai Dapur
    $conn->query("INSERT INTO financial_transactions (organization_id, transaction_date, description, debit_account_id, credit_account_id, amount, created_by) 
        VALUES (2, DATE_SUB(CURDATE(), INTERVAL 24 DAY), 'Tarik Kas Bank ke Kas Tunai Dapur', 1, 2, 20000000.00, 4)"); // Debit Kas Tunai (1), Kredit Bank (2)
    ...
    foreach ($belanja as $b) {
        ...
        $conn->query("INSERT INTO financial_transactions (organization_id, transaction_date, description, debit_account_id, credit_account_id, amount, created_by) 
            VALUES (2, '{$trxDate}', '{$b['desc']}', 4, 1, {$b['amount']}, 4)"); // Debit Biaya Bahan Baku (4), Kredit Kas (1)
    }
    ...
    foreach ($expenses as $exp) {
        ...
        $conn->query("INSERT INTO financial_transactions (organization_id, transaction_date, description, debit_account_id, credit_account_id, amount, created_by) 
            VALUES (2, '{$expDate}', 'Jurnal: {$exp['desc']}', {$debitAcc}, 1, {$exp['amount']}, 4)");
    }
